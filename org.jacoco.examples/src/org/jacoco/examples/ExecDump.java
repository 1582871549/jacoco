...
public final class ExecDump {
    ...
    private void dump(final String file) throws IOException {
        ...
        ISessionInfoVisitor infoVisitor = new ISessionInfoVisitor() {
            public void visitSessionInfo(SessionInfo info) {
                out.printf("Session \"%s\": %s - %s%n", info.getId(), new Date(info.getStartTimeStamp()), new Date(info.getDumpTimeStamp()));
            }
        };

        reader.setSessionInfoVisitor(infoVisitor);

        IExecutionDataVisitor dataVisitor = new IExecutionDataVisitor() {
        ...
        reader.setExecutionDataVisitor(dataVisitor);
        reader.read();
        in.close();
        out.println();
    }
